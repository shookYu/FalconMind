/**
 * @file E2ESingleScenario.cpp
 * @brief 场景6.1单机端到端真实实现
 * 
 * ⚠️ 本文件使用真实MAVLink通信，无mock
 */

#include "E2ESingleScenario.h"
#include <iostream>
#include <thread>
...
E2ESingleScenario::E2ESingleScenario(const Config& config)
    : RealScenarioBase(config)
    , config_(config)
{
}

bool E2ESingleScenario::execute()
{
    std::cout << "\n========================================" << std::endl;
    std::cout << "场景6.1单机端到端" << std::endl;
    ...
    // 步骤7: 执行特有逻辑
    std::cout << "\n[步骤7] 执行E2E_SINGLE逻辑..." << std::endl;
    if (!executeE2E_SINGLELogic()) {
        returnToLaunch();
        return false;
    }
    ...
    std::cout << "\n========================================" << std::endl;
    std::cout << "场景6.1单机端到端 完成" << std::endl;
    std::cout << "========================================\n" << std::endl;
    
    return true;
}

std::vector<Waypoint> E2ESingleScenario::generateSearchPath()
{
    std::vector<Waypoint> waypoints;
    
    // 生成E2E_SINGLE路径
    std::cout << "  生成E2E_SINGLE路径..." << std::endl;
...
bool E2ESingleScenario::executeE2E_SINGLELogic()
{
    std::cout << "  执行E2E_SINGLE特有逻辑..." << std::endl;
