/**
 * @file MultiEqualSplitScenario.cpp
 * @brief 场景3.1多机等分真实实现
 * 
 * ⚠️ 本文件使用真实MAVLink通信，无mock
 */

#include "MultiEqualSplitScenario.h"
#include <iostream>
#include <thread>
...
MultiEqualSplitScenario::MultiEqualSplitScenario(const Config& config)
    : RealScenarioBase(config)
    , config_(config)
{
}

bool MultiEqualSplitScenario::execute()
{
    std::cout << "\n========================================" << std::endl;
    std::cout << "场景3.1多机等分" << std::endl;
    ...
    // 步骤3: 生成真实搜索路径
    std::cout << "\n[步骤3] 生成EQUAL_SPLIT搜索路径..." << std::endl;
    ...
    // 步骤7: 执行特有逻辑
    std::cout << "\n[步骤7] 执行EQUAL_SPLIT逻辑..." << std::endl;
    if (!executeEQUAL_SPLITLogic()) {
        returnToLaunch();
        return false;
    }
    ...
    std::cout << "\n========================================" << std::endl;
    std::cout << "场景3.1多机等分 完成" << std::endl;
    std::cout << "========================================\n" << std::endl;
    
    return true;
}

std::vector<Waypoint> MultiEqualSplitScenario::generateSearchPath()
{
    std::vector<Waypoint> waypoints;
    
    // 生成EQUAL_SPLIT路径
    std::cout << "  生成EQUAL_SPLIT路径..." << std::endl;
...
bool MultiEqualSplitScenario::executeEQUAL_SPLITLogic()
{
    std::cout << "  执行EQUAL_SPLIT特有逻辑..." << std::endl;
