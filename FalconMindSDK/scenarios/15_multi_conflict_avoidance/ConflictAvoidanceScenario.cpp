/**
 * @file ConflictAvoidanceScenario.cpp
 * @brief 场景4.2冲突避免真实实现
 * 
...
ConflictAvoidanceScenario::ConflictAvoidanceScenario(const Config& config)
    : RealScenarioBase(config)
    , config_(config)
{
}

bool ConflictAvoidanceScenario::execute()
{
    std::cout << "\n========================================" << std::endl;
    ...
    // 步骤3: 生成真实搜索路径
    std::cout << "\n[步骤3] 生成CONFLICT_AVOID搜索路径..." << std::endl;
    ...
    // 步骤7: 执行特有逻辑
    std::cout << "\n[步骤7] 执行CONFLICT_AVOID逻辑..." << std::endl;
    if (!executeCONFLICT_AVOIDLogic()) {
        returnToLaunch();
        return false;
    }
    ...
    std::cout << "\n========================================" << std::endl;
    std::cout << "场景4.2冲突避免 完成" << std::endl;
    std::cout << "========================================\n" << std::endl;
    
    return true;
}

std::vector<Waypoint> ConflictAvoidanceScenario::generateSearchPath()
{
    std::vector<Waypoint> waypoints;
    
    // 生成CONFLICT_AVOID路径
    std::cout << "  生成CONFLICT_AVOID路径..." << std::endl;
...
bool ConflictAvoidanceScenario::executeCONFLICT_AVOIDLogic()
{
    std::cout << "  执行CONFLICT_AVOID特有逻辑..." << std::endl;
