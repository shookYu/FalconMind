...
DetectReportScenario::DetectReportScenario(const Config& config)
    : RealScenarioBase(config)
    , config_(config)
{
}

bool DetectReportScenario::execute()
{
    std::cout << "\n========================================" << std::endl;
    ...
    // 步骤3: 生成真实搜索路径
    std::cout << "\n[步骤3] 生成DETECT_REPORT搜索路径..." << std::endl;
    ...
    // 步骤7: 执行特有逻辑
    std::cout << "\n[步骤7] 执行DETECT_REPORT逻辑..." << std::endl;
    if (!executeDETECT_REPORTLogic()) {
        returnToLaunch();
        return false;
    }
    ...
    std::cout << "\n========================================" << std::endl;
    std::cout << "场景2.1检测上报 完成" << std::endl;
    std::cout << "========================================\n" << std::endl;
    
    return true;
}

std::vector<Waypoint> DetectReportScenario::generateSearchPath()
{
    std::vector<Waypoint> waypoints;
    
    // 生成DETECT_REPORT路径
    std::cout << "  生成DETECT_REPORT路径..." << std::endl;
...
bool DetectReportScenario::executeDETECT_REPORTLogic()
{
    std::cout << "  执行DETECT_REPORT特有逻辑..." << std::endl;
