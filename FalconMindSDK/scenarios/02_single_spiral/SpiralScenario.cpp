/**
 * @file SpiralScenario.cpp
 * @brief 场景1.2真实版本 - 螺旋搜索实现
 * 
 ...
 * @param config 场景配置
 */
SpiralScenario::SpiralScenario(const SpiralConfig& config)
    : config_(config)
    , connected_(false)
    , armed_(false)
 ...
 * @brief 连接飞控
 */
bool SpiralScenario::connectVehicle()
{
    try {
 ...
 * @brief 断开飞控连接
 */
void SpiralScenario::disconnectVehicle()
{
    if (mavlinkClient_) {
 ...
 * @brief 检查飞控健康状态
 */
bool SpiralScenario::checkVehicleHealth()
{
 ...
 * @brief 坐标转换：米偏移转经纬度
 */
std::pair<double, double> SpiralScenario::offsetToLatLon(double lat, double lon, float dx, float dy)
{
 ...
 * 3. 直到半径达到搜索边界
 */
std::vector<Waypoint> SpiralScenario::generateSearchPath()
{
    ...
    // 阿基米德螺旋参数
    float a = config_.turnRadius / 2.0f;  // 起始半径
    float b = config_.turnRadius / (2.0f * PI);  // 螺旋系数，控制间距
    ...
    std::cout << "  搜索半径: " << maxRadius << "m" << std::endl;
    std::cout << "  螺旋起始半径: " << a << "m" << std::endl;
    std::cout << "  螺旋间距: " << config_.turnRadius << "m" << std::endl;
 ...
 * @brief 上传航点任务到飞控
 */
bool SpiralScenario::uploadMission(const std::vector<Waypoint>& waypoints)
{
    if (!connected_ || !mavlinkClient_) {
 ...
 * @brief 解锁电机
 */
bool SpiralScenario::armVehicle()
{
    if (!connected_ || !mavlinkClient_) {
        return false;
    }
 ...
 * @brief 上锁电机
 */
bool SpiralScenario::disarmVehicle()
{
    if (!connected_ || !mavlinkClient_) {
        return false;
    }
 ...
 * @brief 起飞到指定高度
 */
bool SpiralScenario::takeoff(float altitude)
{
    if (!connected_ || !mavlinkClient_) {
        return false;
    }
 ...
 * @brief 开始执行任务
 */
bool SpiralScenario::startMission()
{
    if (!connected_ || !mavlinkClient_) {
        return false;
    }
 ...
 * @brief 监控任务执行
 */
void SpiralScenario::monitorMissionExecution()
{
 ...
 * @brief 返航
 */
bool SpiralScenario::returnToLaunch()
{
    if (!connected_ || !mavlinkClient_) {
        return false;
    }
 ...
 * @brief 报告进度
 */
void SpiralScenario::reportProgress(int current, int total, const std::string& status)
{
 ...
 * @brief 打印飞控信息
 */
void SpiralScenario::printVehicleInfo()
{
