...
class GeofenceMonitoringNode : public Node {
public:
    explicit GeofenceMonitoringNode(const std::string& nodeId) 
        : Node(nodeId), active_(false) {
        auto inPad = std::make_shared<Pad>("in", PadType::Sink);
        addPad(inPad);
        std::cout << "[GeofenceMonitoringNode] 初始化完成" << std::endl;
    }
    
    bool configure(const std::unordered_map<std::string, std::string>& params) override {
        std::cout << "[GeofenceMonitoringNode] 配置完成" << std::endl;
        return true;
    }
    
    bool start() override {
        std::cout << "[GeofenceMonitoringNode] 启动" << std::endl;
        active_ = true;
        return true;
    }
    
    void stop() override {
        std::cout << "[GeofenceMonitoringNode] 停止" << std::endl;
        active_ = false;
    }
    ...
    void process() override {
        ...
        count++;
        if (count % 3 == 0) {
            std::cout << "[GeofenceMonitoringNode] 执行处理周期 #" << count << std::endl;
        }
    }
...
int main() {
    std::cout << "================================================================================" << std::endl;
    std::cout << "              FalconMindSDK P2示例: GeofenceMonitoring" << std::endl;
    std::cout << "================================================================================" << std::endl;
    
    PipelineConfig config{"GeofenceMonitoring_pipeline", "GeofenceMonitoring流程", "真实SDK实现"};
    auto pipeline = std::make_shared<Pipeline>(config);
    std::cout << "[1] 创建Pipeline: " << pipeline->id() << std::endl;
    
    auto node = std::make_shared<GeofenceMonitoringNode>("GeofenceMonitoring_controller");
