...
createApp({
  setup() {
    ...
    // ========== 初始化管理器 ==========
    // 注意：cesiumManager 需要先创建，因为 locationManager 需要它
    const cesiumManager = createCesiumManager(state, null, viewerRef, cameraAdjustmentFrameRef);
    const locationManager = createLocationManager(state, viewerRef, cesiumManager);
    // 更新 cesiumManager 的 locationManager 引用（如果需要）
    ...
    onMounted(async () => {
      ...
      if (window.EntityBatcher) {
        ...
        // EntityBatcher 不需要 start() 方法，它会在 queueUpdate 时自动开始处理
