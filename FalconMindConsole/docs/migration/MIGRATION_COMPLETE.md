# ClusterCenter → FalconMindConsole 完整迁移总结
...
现在 FalconMindConsole 集成了完整的 ClusterCenter 功能，API 列表如下：
...
ClusterCenter 的核心功能已完整迁移到 FalconMindConsole 中。现在可以通过单一的 FalconMindConsole 服务管理无人机集群的所有功能，无需再单独部署 ClusterCenter。
