...
FalconMind 采用**四层架构**设计，各部分职责清晰、松耦合：
...
│                                 │                                           │
│  ┌──────────────────────────────┼──────────────────────────────────────┐   │
│  │  FalconMindBuilder (边缘开发层) - "可视化工作室"                     │   │
│  │  ┌───────────────────────────┼──────────────────────────────────┐   │   │
│  │  │ 职责：可视化编排、即时部署、配置调试                           │   │   │
│  │  │ 方式：部署到 UAV，浏览器访问，拖拽编排                          │   │   │
│  │  │ 技术：Vue3 + FastAPI + SQLite + Cesium                        │   │   │
│  │  │ 位置：UAV 边缘设备 (RK3588等)                                  │   │   │
│  │  │ 特点：不依赖地面站，直连 UAV 即可开发                          │   │   │
│  │  └───────────────────────────┼──────────────────────────────────┘   │   │
│  └──────────────────────────────┼──────────────────────────────────────┘   │
│                                 │                                           │
│  ┌──────────────────────────────┼──────────────────────────────────────┐   │
│  │  NodeAgent (边缘自治层) - "自主大脑"                                 │   │
│  │  ┌───────────────────────────┼──────────────────────────────────┐   │   │
│  │  │ 职责：离线决策、任务执行、状态管理                             │   │   │
│  │  │ 方式：部署到无人机，自动运行，断网自治                          │   │   │
│  │  │ 技术：C++17 + SQLite + MAVLink                                 │   │   │
│  │  │ 位置：每架 UAV 一个 NodeAgent                                  │   │   │
│  │  └───────────────────────────┼──────────────────────────────────┘   │   │
│  │                              │                                       │   │
│  │  ┌───────────────────────────┴──────────────────────────────────┐   │   │
...
| 组件 | 运行位置 | 核心职责 | 使用方式 | 网络依赖 |
|------|----------|----------|----------|----------|
| **Console** | 地面站/云端 | **指挥中心**：集群监控、任务管理、数据分析 | 打开浏览器，设计任务流程，监控机群 | 需要网络 |
| **Builder** | UAV 边缘 | **可视化工作室**：零代码编排、即时部署、现场调试 | 浏览器访问 UAV IP，拖拽编排，即时生效 | **可选**（直连 UAV） |
| **NodeAgent** | UAV 上 | **自主大脑**：离线决策、任务执行、状态管理 | 部署到每架无人机，自动运行 | **无需**（离线自治） |
| **SDK** | 编译进组件 | **能力库**：AI感知、飞行控制、任务规划 | 调用API实现功能，或开发新插件 | 组件决定 |
